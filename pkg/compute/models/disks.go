...
func (manager *SDiskManager) AutoDiskOverdueSnapshot(ctx context.Context, userCred mcclient.TokenCredential, isStart bool) {
	disks, err := manager.GetNeedAutoSnapshotDisks()
	if err != nil {
		log.Errorf("Get auto snapshot disks id failed: %s", err)
		return
	...
	for i := range disks {
		...
		if len(disk.ExternalId) == 0 {
			err = disk.validateDiskAutoCreateSnapshot()
			if err != nil {
				log.Errorf("disk %s validate auto create snapshot failed: %s", disk.Id, err)
				continue
			}
		}
		policy, err := disks[i].GetSnapshotPolicy()
		if err != nil {
			log.Errorf("disk %s GetSnapshotPolicy failed: %s", disk.Id, err)
			continue
		}

		now := time.Now()
		if err = disk.CleanOverdueSnapshots(ctx, userCred, policy, now); err != nil {
