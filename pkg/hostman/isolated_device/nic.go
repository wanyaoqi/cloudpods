...
func ensureNumvfsEqualTotalvfs(nicDir string) error {
	...
	if numvfs != totalvfs {
		return fileutils2.FilePutContents(sriovNumvfs, fmt.Sprintf("%s\n", totalvfs), false)
	}
	return nil
}
...
func getOvsOffloadNics(hostNics []HostNic) ([]*sOvsOffloadNicDevice, error) {
	...
	for i := range nics {
		...
		if strings.TrimSpace(linkMode) != "switchdev" {
			...
			err = fileutils2.FilePutContents(linkMode, "switchdev\n", false)
			if err != nil {
		...
		for _, line := range strings.Split(string(outs), "\n") {
			...
			if len(sp) != 2 {
				return nil, errors.Errorf("%s bad portname", vfName, portname)
			}
