...
func (s *SKVMGuestInstance) reallocateMigrateNumaNodes() error {
	nodeNumaCpus, err := s.manager.cpuSet.AllocCpusetWithNodeCount(int(s.Desc.Cpu), s.Desc.Mem*1024, s.Desc.MemDesc.GuestNumaNodeCount())
	if err != nil {
...
func (s *SKVMGuestInstance) initLiveDescFromSourceGuest(srcDesc *desc.SGuestDesc) error {
	...
	if len(s.Desc.CpuNumaPin) > 0 {
		...
		nodeNumaCpus, err := s.manager.cpuSet.AllocCpusetWithNodeCount(int(srcDesc.Cpu), srcDesc.Mem*1024, srcDesc.MemDesc.GuestNumaNodeCount())
		if err != nil {
			return errors.Wrap(err, "AllocCpusetWithNodeCount")
		}
