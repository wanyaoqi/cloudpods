...
func (s *SKVMGuestInstance) DeleteSnapshot(ctx context.Context, delParams *SDeleteDiskSnapshot) (jsonutils.JSONObject, error) {
	if !utils.IsInStringArray(delParams.Disk.GetType(), []string{api.STORAGE_LOCAL, api.STORAGE_LVM, api.STORAGE_SLVM}) {
		...
		return res, delParams.Disk.DeleteSnapshot(delParams.DeleteSnapshot, delParams.PreviousSnapshot, delParams.NextSnapshot, delParams.EncryptInfo)
	}
	return s.ExecDeleteSnapshotTask(ctx, delParams.Disk, delParams.DeleteSnapshot, delParams.PreviousSnapshot, delParams.NextSnapshot, delParams.EncryptInfo,
...
func (s *SKVMGuestInstance) ExecDeleteSnapshotTask(
	...
	if s.IsRunning() {
		if s.isLiveSnapshotEnabled() {
			task := NewGuestSnapshotDeleteTask(ctx, s, disk, deleteSnapshot, previousSnapshot, nextSnapshot, encryptInfo)
		...
		return s.deleteStaticSnapshotFile(ctx, disk, deleteSnapshot, previousSnapshot, nextSnapshot, encryptInfo)
	}
}

func (s *SKVMGuestInstance) deleteStaticSnapshotFile(
	ctx context.Context, disk storageman.IDisk, deleteSnapshot, previousSnapshot, nextSnapshot string, encryptInfo apis.SEncryptInfo,
) (jsonutils.JSONObject, error) {
	if err := disk.DeleteSnapshot(deleteSnapshot, previousSnapshot, nextSnapshot, encryptInfo); err != nil {
		log.Errorln(err)
		return nil, err
	}
	res := jsonutils.NewDict()
