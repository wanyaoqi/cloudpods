...
func (s *SGuestLiveMigrateTask) mirrorDisks(res string) {
	...
	if utils.IsInStringArray(s.Desc.Disks[i].StorageType, api.STORAGE_LOCAL_TYPES) {
		s.Monitor.DriveMirror(
			s.mirrorDisks, fmt.Sprintf("drive_%d", s.Desc.Disks[i].Index),
			fmt.Sprintf("nbd:%s:%d:exportname=drive_%d", s.params.DestIp, s.params.NbdServerPort, s.Desc.Disks[i].Index),
			"top", s.Desc.Disks[i].Format, true, false,
		)
	} else {
		s.mirrorDisks("")
	}
