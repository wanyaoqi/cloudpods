...
func (d *SRBDDisk) DeleteSnapshot(snapshotId, previousSnapshot, nextSnapshot string, encryptInfo apis.SEncryptInfo) error {
	storage := d.Storage.(*SRbdStorage)
...
func (d *SRBDDisk) DiskDeleteSnapshot(ctx context.Context, params interface{}) (jsonutils.JSONObject, error) {
	...
	err := d.DeleteSnapshot(snapshotId, "", "", apis.SEncryptInfo{})
	if err != nil {
		return nil, err
	} else {
